...
Контекст:
	...
	И Я запоминаю в переменную "НомерДокумента" значение '000000008'
...
//	И Я закрываю окно 'Обслуживание клиента $НомерДокумента$ от *'
//	Тогда открылось окно 'Обслуживание клиента'
	...
	И пока в таблице "ВыполненныеРаботы" количество строк ">" 0 Тогда
		И в таблице "ВыполненныеРаботы" я выделяю все строки
		И в таблице 'ВыполненныеРаботы' я удаляю строку
...
* Тогда таблица "Список" стала равной:
	Тогда таблица "Список" стала равной:
		...
		| '$Период$' | '$ЗаголовокОкна$' | ''               | '1'            | '$Специалист$' | '1'              |
		| '$Период$' | '$ЗаголовокОкна$' | ''               | '2'            | '$Специалист$' | '1'              |
		| '$Период$' | '$ЗаголовокОкна$' | ''               | '3'            | '$Специалист$' | '1'              |
		| '$Период$' | '$ЗаголовокОкна$' | ''               | '4'            | '$Специалист$' | '1'              |
* Потом я проверяю записи в регистре Выполненные Коиенту работы
	...
	Тогда таблица "Список" стала равной:
		...
		| '$Период$' | '$ЗаголовокОкна$' | '2 000,00'       | '1'            | 'ООО "Гамма"' | 'Договор №5' | '4'                |
	И В текущем окне я нажимаю кнопку командного интерфейса 'Выполненные сотрудником работы'
...
* Я вижу все заполненные мною поля в  документе
	И таблица "Список" содержит строки по шаблону:
		...
		| '$НомерДокумента$' | '$Специалист$' | 'Да'       | '18:00:00'                     | '03.07.2025'            | 'Востановить работоспособность системы' | '12:00:00'                  | 'Работа выполнена' |
