...
Процедура СформироватьДвижения()
	...
	СформироватьДвиженияПроцентОтвыполненныхРабот();
...
Процедура СформироватьДвиженияПроцентОтвыполненныхРабот()
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВКМ_ВыполненныеСотрудникомРаботыОстаткиИОбороты.Сотрудник КАК Сотрудник,
	|	ЕСТЬNULL(ВКМ_ВыполненныеСотрудникомРаботыОстаткиИОбороты.СуммаКОплатеОборот, 0) КАК Сумма,
	|	ВКМ_ВыполненныеСотрудникомРаботыОстаткиИОбороты.Сотрудник.Подразделение КАК СотрудникПодразделение
	|ИЗ
	|	РегистрНакопления.ВКМ_ВыполненныеСотрудникомРаботы.ОстаткиИОбороты(&ДатаНачала, &ДатаОкончания,,,) КАК
	|		ВКМ_ВыполненныеСотрудникомРаботыОстаткиИОбороты";
	...
	Пока Выборка.Следующий() Цикл
		...
		Движение.Подразделение = Перечисления.ВКМ_Подразделения.Специалист;
...
Процедура СформироватьДвиженияУдержания()
	...
	ТЗ.Свернуть("Сотрудник,Подразделение");
...
Процедура РасчитатьДвиженияУдержания()
    ...
    |	ВЫБОР
    |		КОГДА ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.РегистраторРазрез = ВКМ_Удержания.Регистратор
    |			ТОГДА ЕСТЬNULL(ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.СуммаБаза, 0)
    |		ИНАЧЕ 0
    |	КОНЕЦ КАК СуммаОН,
    |	ВЫБОР
    |		КОГДА ВКМ_УдержанияБазаВКМ_ДополнительныеНачисления.РегистраторРазрез = ВКМ_Удержания.Регистратор
...
//	Запрос.УстановитьПараметр("ВидРасчета", ПланыВидовРасчета.ВКМ_Удержания.НДФЛ);
	...
	Пока Выборка.Следующий() Цикл
		СуммаБазы = Выборка.СуммаОН + Выборка.СуммаДН;
	   ...
	   ДвижениеУдержание.Сторно = Ложь;
  	   ДвижениеУдержание.СуммаУдержания = ОКР((Выборка.СуммаОН + Выборка.СуммаДН) * 0.13,2);
...
//		|	СУММА(ЕСТЬNULL(ВКМ_ОсновныеНачисления.Сумма, 0)) КАК СуммаНачисления,
//		|	СУММА(ЕСТЬNULL(ВКМ_ВыполненныеСотрудникомРаботыОбороты.СуммаКОплатеОборот, 0)) КАК СуммаПроцент
//		|ИЗ
...
//		|		ПОЛНОЕ СОЕДИНЕНИЕ РегистрРасчета.ВКМ_ОсновныеНачисления КАК ВКМ_ОсновныеНачисления
...
//		|	ВКМ_ОсновныеНачисления.Сотрудник";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Движение.Период = Дата;
		Движение.Сотрудник = ВыборкаДетальныеЗаписи.Сотрудник;
		Движение.Сумма = ВыборкаДетальныеЗаписи.СуммаНачисления; // + ВыборкаДетальныеЗаписи.СуммаПроцент ;
	КонецЦикла;
КонецПроцедуры
...
Процедура РассчитатьОтпуск()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Движение.Сумма = (ВыборкаДетальныеЗаписи.БазаДоп + ВыборкаДетальныеЗаписи.БазаОсн)
			/ ВыборкаДетальныеЗаписи.ДнейРабочих * КалендарныеДни;
