...
Процедура СформироватьДвиженияУдержания()
	...
	|	СУММА(ЕСТЬNULL(ВКМ_ОсновныеНачисления.Сумма, 0)) КАК СуммаОН,
	|	СУММА(ЕСТЬNULL(ВКМ_ДополнительныеНачисления.Сумма, 0)) КАК СуммаДН
	|ИЗ
	|	РегистрРасчета.ВКМ_ДополнительныеНачисления КАК ВКМ_ДополнительныеНачисления
	|		ПОЛНОЕ СОЕДИНЕНИЕ РегистрРасчета.ВКМ_ОсновныеНачисления КАК ВКМ_ОсновныеНачисления
	|		ПО ВКМ_ОсновныеНачисления.Регистратор = ВКМ_ДополнительныеНачисления.Регистратор
	|		И ВКМ_ОсновныеНачисления.Сотрудник = ВКМ_ДополнительныеНачисления.Сотрудник
	|ГДЕ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если (ВыборкаДетальныеЗаписи.СуммаОН + ВыборкаДетальныеЗаписи.СуммаДН) <= 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		ДвижениеУдержание.СуммаУдержания = ОКР((ВыборкаДетальныеЗаписи.СуммаОН + ВыборкаДетальныеЗаписи.СуммаДН) * 0.13,
			2);
	КонецЦикла;
	Движения.ВКМ_Удержания.Записать();
